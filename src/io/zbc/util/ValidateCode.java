...
public class ValidateCode {
    ...
    public static String generateTextCode(int type, int length, String exChars) {
        ...
        case TYPE_NUM_ONLY:
            while (i < length) {
                int t = r.nextInt(10);
                if (exChars == null || exChars.indexOf(String.valueOf(t)) < 0) {// 排除特殊字符
                    code.append(t);
                    i++;
                }
            }
     ...
     * @return
     *//*
       * public void getValidateCode(){ // 设置浏览器不缓存本页
       * this.getResponse().setHeader("Cache-Control", "no-cache"); //
       * 生成验证码，写入用户session String verifyCode =
       * ValidateCode.generateTextCode(ValidateCode.TYPE_ALL_MIXED, 4, null);
       * this.getSession().setAttribute("validateCode", verifyCode); //
       * 输出验证码给客户端 this.getResponse().setContentType("image/jpeg");
       * BufferedImage bim = ValidateCode.generateImageCode(verifyCode, 60, 20,
       * 3, true, Color.WHITE, Color.BLACK, null); try { ImageIO.write(bim,
       * "JPEG", this.getResponse().getOutputStream());
       * this.getResponse().getOutputStream().close(); } catch (IOException e) {
       * e.printStackTrace(); } }
       */
